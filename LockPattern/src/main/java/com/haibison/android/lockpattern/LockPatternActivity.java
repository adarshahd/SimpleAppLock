...
public class LockPatternActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        unlockSelf = getIntent().getBooleanExtra("unlock_self",true);
        changeLock = getIntent().getBooleanExtra("change_lock",false);
        if(!unlockSelf){
            mPackage = getIntent().getStringExtra(Intent.EXTRA_TEXT);
        }
    ...
    private void doComparePattern(final List<Cell> pattern) {
        ...
        new LoadingDialog<Void, Void, Boolean>(this, false) {
            ...
            protected void onPostExecute(Boolean result) {
                ...
                if (result) {
                    if(!unlockSelf){
                        PreferenceManager.getDefaultSharedPreferences(mContext).edit().putBoolean("unlocked",true).commit();
                        PreferenceManager.getDefaultSharedPreferences(mContext).edit().putString("last_app",mPackage).commit();
